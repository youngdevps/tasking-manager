...
def upgrade():
    conn = op.get_bind()
    fetch_country_combinations = "select distinct(unnest(country)) from projects where country is not null;"
    country_combinations = conn.execute(fetch_country_combinations)
    for country in country_combinations:
        country = country[0]
        print(country)
        url = "https://nominatim.openstreetmap.org/search?country=" + country + "&format=jsonv2&accept-language=en"
        print(url)
